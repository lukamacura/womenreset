persona: "exercise"
topic: "Exercise in Menopause"
subtopic: "Weekly beginner exercise plan"

content_text: |

        This plan builds strength, mobility, and stamina without overwhelming your body.

      1. 📅 **Day 1 – Strength + Core (40–45 min):** Bodyweight/bands (squats, push-ups, glute bridges, rows) + pelvic floor. 
      2. 📅 **Day 2 – Zone 2 Cardio + Stretch (30–40 min):** Brisk walk or cycle + gentle yoga.  
      3. 📅 **Day 3 – Intro Intervals (20–25 min):** 3 × 15–20 sec faster walk/jog + 90 sec recovery.  
      4. 📅 **Day 4 – Recovery & Balance (20–25 min):** Gentle yoga or tai chi + balance drills.  
      5. 📅 **Day 5 – Strength + Light Plyo (35–40 min):** Squats, presses, rows + optional low-impact jumps.  
      6. 📅 **Day 6 – Zone 2 Cardio (35–45 min):** Steady walk or cycle + stretch.  
      7. 📅 **Day 7 – Active Recovery (20–30 min):** Easy walk, yoga, or foam rolling.

      
motivation_nudge:"❤️ Start where you are, keep it doable, and let the wins stack."
    
follow_up_question: "Would you like to learn why recovery matters more now, or learn more about each pillar of menopause fitness?"
follow_up_links:
  -persona: "exercise"
  topic: "Exercise in Menopause"
  subtopic: "Recovery & Rest"
  label: "Recovery & Rest"
  -persona: "exercise"
  topic: "Exercise in Menopause"
  subtopic: "Strength Training"
  label: "Strength Training"
  -persona: "exercise"
  topic: "Exercise in Menopause"
  subtopic: "Sprint Interval Training (SIT)"
  label: "Sprint Interval Training (SIT)"
  -persona: "exercise"
  topic: "Exercise in Menopause"
  subtopic: "Plyometric Training (Jump Training)"
  label: "Plyometric Training (Jump Training)"
  -persona: "exercise"
  topic: "Exercise in Menopause"
  subtopic: "Zone 2 Training (Cardio)"
  label: "Zone 2 Training (Cardio)"
  -persona: "exercise"
  topic: "Exercise in Menopause"
  subtopic: "Flexibility & Mobility"
  label: "Flexibility & Mobility"
  -persona: "exercise"
  topic: "Exercise in Menopause"
  subtopic: "Balance & Stability Training"
  label: "Balance & Stability Training"
  -persona: "exercise"
  topic: "Exercise in Menopause"
  subtopic: "Core & Pelvic Floor Training"
  label: "Core & Pelvic Floor Training"

intent_patterns:
  - "What does a beginner exercise week look like in menopause?"
  - "Can you give me a weekly workout plan for beginners over 40?"
  - "How should I structure my first week of exercise?"
  - "What's a complete beginner schedule for menopause fitness?"
  - "Day-by-day beginner workout plan menopause"
  - "I'm new to exercise, what should my week include?"
  - "Beginner weekly routine with strength and cardio"
  - "How many days should a beginner exercise?"
  - "What activities go on which days for beginners?"
  - "Simple week-long exercise template for women over 40"
  - "How much time per day for beginner workouts?"
  - "Is 7 days too much for a beginner?"
  - "What's active recovery vs rest day?"
  - "How to balance strength, cardio, and rest as beginner?"
  - "Complete week beginner menopause plan"
  - "Starting from zero - what does week one look like?"
  - "Beginner schedule that covers all bases"
  - "How long should each workout be for beginners?"
  - "What order to do different exercises weekly?"
  - "Beginner plan that builds slowly"

keywords:
  - "beginner exercise plan"
  - "weekly beginner schedule"
  - "beginner workout week"
  - "day-by-day plan beginners"
  - "weekly routine beginners"
  - "starting exercise menopause"
  - "beginner weekly template"
  - "7-day beginner plan"
  - "complete week beginners"
  - "beginner schedule structure"
  - "how many days beginners"
  - "beginner workout duration"
  - "strength cardio balance"
  - "active recovery day"
  - "beginner rest days"
  - "consistency over intensity"
  - "small start perfect"
  - "build confidence first"
  - "gentle progression beginners"
  - "sustainable beginner routine"
